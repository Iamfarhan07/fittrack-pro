...
import { Save, Dumbbell, Trophy, Plus, Trash2, ArrowRight, History as HistoryIcon, CheckCircle2, X } from 'lucide-react';
...
export default function WorkoutTracker({ userId }: { userId: string }) {
  ...
  async function checkOrCreateWorkout() {
    ...
    let { data, error: selectError } = await supabase.from('workouts').select('id').eq('user_id', userId).eq('date', today).single();
    if (!data) {
      const { data: newWorkout, error: insertError } = await supabase.from('workouts').insert([{ user_id: userId, date: today, split_type: 'Push' }]).select().single();
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 w-full max-w-md shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
              ...
              <button onClick={() => setShowReport(false)} className="w-full bg-white text-black py-3.5 rounded-xl font-bold transition-transform active:scale-95 shadow-sm">
                Close Report
              </button>
            </div>
          </div>
